            ...
            @forelse($profiles ?? [] as $profile)
                <tr>
                    ...
                    <td class="actions-cell">
                        <a href="{{ route('admin.user-profiles.edit', $profile['id']) }}" class="link-btn link-edit">
                        ...
                        <form action="{{ route('admin.user-profiles.destroy', $profile['id']) }}" method="POST"
                              class="inline-form"
                              onsubmit="return confirm('Delete {{ $profile['username'] }}? This cannot be undone.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                    class="link-btn link-del" {{ ! empty($profile['student']) ? 'disabled' : '' }}>
                                <svg width="13" height="13" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
