...
@php($teacher = $teacher ?? session('teacher'))
...
@section('content')
    <div class="detail-card" style="margin-bottom: 2rem;">
        <x-profile-picture-upload
            ...
            :initials="strtoupper(substr($teacher->fname ?? 'T', 0, 1).substr($teacher->lname ?? 'C', 0, 1))"
            :display-name="trim(($teacher->fname ?? 'Teacher').' '.($teacher->lname ?? ''))"
            :upload-route="route('teacher.upload-profile-picture')"
            input-id="teacherProfilePictureInput"
            container-id="teacherProfilePictureContainer"
            image-id="teacherProfileImage"
            initials-id="teacherProfileInitials"
            status-id="teacherUploadStatus"
        />
    </div>
